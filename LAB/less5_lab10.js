...
test.describe('Проверка стиля в Chrome', function () {
    var driver;
    ...
    test.it('Проверяем', function() {
        ...
        driver.findElement(By.css("#box-campaigns > div > div > div > a > div.info > div.price-wrapper > strong"))
            .then(function (elem) {
                ...
                elem.getCssValue("font-weight").then(function (attr) {
                    assert.ok(attr =="bold", "Проверка - шрифт акционной цены bold на карточке");
                    return attr; });
            })
        ...
        driver.findElement(By.css("#box-product div.price-wrapper strong"))
            .then(function (elem) {
                ...
                elem.getCssValue("font-size").then(function (attr) {
                    salePriceSize = parseFloat(attr);
                    assert.ok( salePriceSize > priceSize, "Проверка - размер шрифта акционной цены и основной цены на странице");
                    return salePriceSize; });

                elem.getCssValue("font-weight").then(function (attr) {
                    assert.ok(attr =="bold", "Проверка - шрифт акционной цены bold на странице");
                    return attr; });
            })
